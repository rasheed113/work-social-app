...
class _CreatePostCardState extends State<CreatePostCard> {
  ...
  Future<void> _useLocation() async {
    ...
    if (!mounted) return;
    setState(() { _latitude = position.latitude; _longitude = position.longitude; _locationName = '${position.latitude.toStringAsFixed(5)}, ${position.longitude.toStringAsFixed(5)}'; });
  }

  Future<void> _submit() async {
    setState(() => _loading = true);
    try {
      await PostRepository(SupabaseClientProvider.client).createPost(PostDraft(content: _controller.text, latitude: _latitude, longitude: _longitude, locationName: _locationName, files: List.unmodifiable(_files)));
      _controller.clear();
      setState(() { _files.clear(); _latitude = null; _longitude = null; _locationName = null; });
  ...
  Widget build(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
    ...
    if (_files.isNotEmpty) Align(alignment: Alignment.centerLeft, child: Wrap(spacing: 8, children: [for (final file in _files) Chip(label: Text(file.path.split('/').last), onDeleted: () => setState(() => _files.remove(file)))])),
    ...
    Row(children: [
      ...
      const Spacer(),
      FilledButton(onPressed: _loading ? null : _submit, child: _loading ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('Post')),
    ]),
  ])));
}
