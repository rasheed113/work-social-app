...
class WebRtcCallEngine(private val context: Context) {
    ...
    suspend fun start(kind: CallKind): SessionDescription {
        createPeerConnection(kind)
        val pc = peerConnection ?: error("WebRTC peer connection was not created")
        val offer = createOffer(pc)
        setLocalDescription(pc, offer)
        return offer
    }

    suspend fun accept(kind: CallKind, sdpType: String, sdp: String): SessionDescription {
        createPeerConnection(kind)
        val pc = peerConnection ?: error("WebRTC peer connection was not created")
        setRemoteDescription(pc, SessionDescription(parseType(sdpType), sdp))
        val answer = createAnswer(pc)
        setLocalDescription(pc, answer)
        return answer
    }

    suspend fun setRemoteAnswer(sdpType: String, sdp: String) {
        val pc = peerConnection ?: error("WebRTC peer connection is not ready")
        setRemoteDescription(pc, SessionDescription(parseType(sdpType), sdp))
    }
        ...
        val observer = object : PeerConnection.Observer {
            ...
            override fun onIceConnectionChange(newState: PeerConnection.IceConnectionState) {
                when (newState) {
                    ...
                    PeerConnection.IceConnectionState.CHECKING -> listener?.onConnecting()
                    PeerConnection.IceConnectionState.FAILED -> listener?.onFailed("ICE failed. The configured STUN/TURN servers could not establish a network path.")
                    else -> Unit
                }
            }
            ...
            override fun onIceCandidate(candidate: IceCandidate) {
                listener?.onIceCandidate(IceCandidateData(candidate.sdpMid, candidate.sdpMLineIndex, candidate.sdp))
            }
            ...
            override fun onConnectionChange(newState: PeerConnection.PeerConnectionState) {
                when (newState) {
                    ...
                    PeerConnection.PeerConnectionState.CONNECTING -> listener?.onConnecting()
                    PeerConnection.PeerConnectionState.FAILED -> listener?.onFailed("WebRTC peer connection failed.")
                    else -> Unit
                }
            }
    ...
    private suspend fun createAnswer(pc: PeerConnection): SessionDescription = suspendCancellableCoroutine { continuation ->
    ...
    private suspend fun setLocalDescription(pc: PeerConnection, description: SessionDescription) = suspendCancellableCoroutine<Unit> { continuation ->
    ...
    private suspend fun setRemoteDescription(pc: PeerConnection, description: SessionDescription) = suspendCancellableCoroutine<Unit> { continuation ->
    ...
    private fun parseType(value: String): SessionDescription.Type = when (value.lowercase()) {
        "offer" -> SessionDescription.Type.OFFER
        "answer" -> SessionDescription.Type.ANSWER
        "pranswer" -> SessionDescription.Type.PRANSWER
        else -> error("Unsupported SDP type: $value")
    }
